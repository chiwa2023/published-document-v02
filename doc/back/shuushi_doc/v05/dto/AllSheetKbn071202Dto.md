...
政治資金収支報告書V5の様式7の12の2（政治資金パーティの対価に係る団体からのあっせんによる収入）の全体データDtoです。

## XML要素

このDtoは、`AllSheet0712PartyMediationDto` の子要素として `KUBUN2` タグで表現され、様式7の12の2の情報を代表（統合）する要素です。内部のシートデータは `SHEET` タグで表現されます。

## プロパティ

| プロパティ名 | XMLタグ名 |                 データ型                  |            説明            |    値の制限    |
| :----------- | :-------- | :---------------------------------------- | :------------------------- | :------------- |
| `list`       | `SHEET`   | `List<Sheet071202MediationPartyGroupDto>` | 様式7の12の2のシートリスト | 未調査／未実装 |

## 子要素の詳細

### `Sheet071202MediationPartyGroupDto` の詳細

`sheet071202MediationPartyGroupDto` は `Template0712SheetDto` を継承し、以下のフィールドを含みます。
