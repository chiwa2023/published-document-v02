...
政治資金収支報告書V5の様式7の18の6（資産の内訳 - 信託）の全体データDtoです。

## XML要素

このDtoは、`AllSheet0718AssetsDto` の子要素として `KUBUN6` タグで表現され、様式7の18の6の情報を代表（統合）する要素です。内部のシートデータは `SHEET` タグで表現されます。

## プロパティ

|           プロパティ名           | XMLタグ名 |             データ型             |         説明         |    値の制限    |
| :------------------------------- | :-------- | :------------------------------- | :------------------- | :------------- |
| `sheet071806TrustAmongAssetsDto` | `SHEET`   | `Sheet071806TrustAmongAssetsDto` | 様式7の18の6のシート | 未調査／未実装 |

## 子要素の詳細

### `Sheet071806TrustAmongAssetsDto` の詳細

`sheet071806TrustAmongAssetsDto` は `Template0718SheetDto` を継承し、以下のフィールドを含みます。
