...
政治資金収支報告書V5の様式7の11の3（政治資金パーティの対価に係る政治団体からの収入）の全体データDtoです。

## XML要素

このDtoは、`AllSheet0711ConsiderationPartyDto` の子要素として `KUBUN3` タグで表現され、様式7の11の3の情報を代表（統合）する要素です。内部のシートデータは `SHEET` タグで表現されます。

## プロパティ

| プロパティ名 | XMLタグ名 |                      データ型                      |            説明            |    値の制限    |
| :----------- | :-------- | :------------------------------------------------- | :------------------------- | :------------- |
| `list`       | `SHEET`   | `List<Sheet071103ConsiderationPartyPoliticOrgDto>` | 様式7の11の3のシートリスト | 未調査／未実装 |

## 子要素の詳細

### `Sheet071103ConsiderationPartyPoliticOrgDto` の詳細

`sheet071103ConsiderationPartyPoliticOrgDto` は `Template0711SheetDto` を継承し、以下のフィールドを含みます。
