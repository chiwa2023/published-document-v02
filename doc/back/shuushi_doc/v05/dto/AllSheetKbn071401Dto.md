...
政治資金収支報告書V5の様式7の14の1（経常経費・政治活動費）の全体データDtoです。

## XML要素

このDtoは、`AllSheet0714ConstsDto` の子要素として `KUBUN1` タグで表現され、様式7の14の1の情報を代表（統合）する要素です。内部のシートデータは `SHEET` タグで表現されます。

## プロパティ

|         プロパティ名         | XMLタグ名 |           データ型           |        説明         |    値の制限    |
| :--------------------------- | :-------- | :--------------------------- | :------------------ | :------------- |
| `sheet071401UtilityCostsDto` | `SHEET`   | `Sheet071401UtilityCostsDto` | 14の1のシートデータ | 未調査／未実装 |

## 子要素の詳細

### `Sheet071401UtilityCostsDto` の詳細

`sheet071401UtilityCostsDto` は `Template0714SheetDto` を継承し、以下のフィールドを含みます。
