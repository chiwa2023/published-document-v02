# AllSheetKbn071509Dto

## 概要

政治資金収支報告書V5の様式7の15の9（その他の経費）の全体データDtoです。

## XML要素

このDtoは、`AllSheet0715ExpenseDto` の子要素として `KUBUN9` タグで表現され、様式7の15の9の情報を代表（統合）する要素です。内部のシートデータは `SHEET` タグで表現されます。

## プロパティ

| プロパティ名 | XMLタグ名 |              データ型               |            説明            |    値の制限    |
| :----------- | :-------- | :---------------------------------- | :------------------------- | :------------- |
| `list`       | `SHEET`   | `List<Sheet071509OtherExpensesDto>` | 様式7の15の9のシートリスト | 未調査／未実装 |

## 子要素の詳細

### `Sheet071509OtherExpensesDto` の詳細

`sheet071509OtherExpensesDto` は `Template0715SheetDto` を継承し、以下のフィールドを含みます。
