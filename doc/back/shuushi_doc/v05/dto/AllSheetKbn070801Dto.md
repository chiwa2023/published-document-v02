...
政治資金収支報告書V5の様式7の8の1（個人によるあっせんの寄付）の全体データDtoです。

## XML要素

このDtoは、`AllSheet0708MediationDto` の子要素として `KUBUN1` タグで表現され、様式7の8の1の情報を代表（統合）する要素です。内部のシートデータは `SHEET` タグで表現されます。

## プロパティ

|          プロパティ名           | XMLタグ名 |            データ型             |        説明         |    値の制限    |
| :------------------------------ | :-------- | :------------------------------ | :------------------ | :------------- |
| `sheet070801MediationPersonDto` | `SHEET`   | `Sheet070801MediationPersonDto` | 様式7の8の1のシート | 未調査／未実装 |

## 子要素の詳細

### `Sheet070801MediationPersonDto` の詳細

`sheet070801MediationPersonDto` は `Template0708SheetDto` を継承し、以下のフィールドを含みます。
