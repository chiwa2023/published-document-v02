...
政治資金収支報告書V5の様式7の18の12（資産の内訳 - 借入金）の全体データDtoです。

## XML要素

このDtoは、`AllSheet0718AssetsDto` の子要素として `KUBUN12` タグで表現され、様式7の18の12の情報を代表（統合）する要素です。内部のシートデータは `SHEET` タグで表現されます。

## プロパティ

|             プロパティ名              | XMLタグ名 |               データ型                |         説明          |    値の制限    |
| :------------------------------------ | :-------- | :------------------------------------ | :-------------------- | :------------- |
| `sheet071812BorrowingsAmongAssetsDto` | `SHEET`   | `Sheet071812BorrowingsAmongAssetsDto` | 様式7の18の12のシート | 未調査／未実装 |

## 子要素の詳細

### `Sheet071812BorrowingsAmongAssetsDto` の詳細

`sheet071812BorrowingsAmongAssetsDto` は `Template0718SheetDto` を継承し、以下のフィールドを含みます。
