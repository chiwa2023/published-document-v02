...
政治資金収支報告書V5の様式7の18の2（資産の内訳 - 建物）の全体データDtoです。

## XML要素

このDtoは、`AllSheet0718AssetsDto` の子要素として `KUBUN2` タグで表現され、様式7の18の2の情報を代表（統合）する要素です。内部のシートデータは `SHEET` タグで表現されます。

## プロパティ

|             プロパティ名             | XMLタグ名 |               データ型               |         説明         |    値の制限    |
| :----------------------------------- | :-------- | :----------------------------------- | :------------------- | :------------- |
| `sheet071802BuildingsAmongAssetsDto` | `SHEET`   | `Sheet071802BuildingsAmongAssetsDto` | 様式7の18の2のシート | 未調査／未実装 |

## 子要素の詳細

### `Sheet071802BuildingsAmongAssetsDto` の詳細

`sheet071802BuildingsAmongAssetsDto` は `Template0718SheetDto` を継承し、以下のフィールドを含みます。
