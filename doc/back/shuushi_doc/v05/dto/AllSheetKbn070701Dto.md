# AllSheetKbn070701Dto

## 概要

政治資金収支報告書V5の様式7の7の1（個人からの寄付収入）の全体データDtoです。

## XML要素

このDtoは、`AllSheet0707DonateDto` の子要素として `KUBUN1` タグで表現され、様式7の7の1の情報を代表する要素です。内部のシートデータは `SHEET` タグで表現されます。

## プロパティ

|         プロパティ名         | XMLタグ名 |           データ型           |       説明        |
| ---------------------------- | --------- | ---------------------------- | ----------------- |
| `sheet070701DonatePersonDto` | `SHEET`   | `Sheet070701DonatePersonDto` | 様式7の7の1データ |

## `Sheet070701DonatePersonDto` およびその子要素

`Sheet070701DonatePersonDto` は `Template0707SheetDto` を継承し、以下のフィールドを含みます。
