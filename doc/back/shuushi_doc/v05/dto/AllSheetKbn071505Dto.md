...
政治資金収支報告書V5の様式7の15の5（政治資金パーティ開催費）の全体データDtoです。

## XML要素

このDtoは、`AllSheet0715ExpenseDto` の子要素として `KUBUN5` タグで表現され、様式7の15の5の情報を代表（統合）する要素です。内部のシートデータは `SHEET` タグで表現されます。

## プロパティ

| プロパティ名 | XMLタグ名 |               データ型                |            説明            |    値の制限    |
| :----------- | :-------- | :------------------------------------ | :------------------------- | :------------- |
| `list`       | `SHEET`   | `List<Sheet071505PartyHostingFeeDto>` | 様式7の15の5のシートリスト | 未調査／未実装 |

## 子要素の詳細

### `Sheet071505PartyHostingFeeDto` の詳細

`sheet071505PartyHostingFeeDto` は `Template0715SheetDto` を継承し、以下のフィールドを含みます。
